...
export default function Chatbot() {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div
        className={`fixed bottom-6 right-6 w-[360px] h-[500px] bg-card border border-border rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] flex flex-col overflow-hidden transition-all duration-300 origin-bottom-right z-50 ${
      ...
      >
        ...
        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-background/80 custom-scrollbar">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] rounded-2xl p-3.5 text-sm shadow-lg ${
                msg.sender === 'user' 
                  ...
                  : msg.isFake 
                    ? 'bg-destructive/20 border border-destructive/30 text-foreground rounded-bl-sm'
                    : msg.isFake === false
                      ? 'bg-success/20 border border-success/30 text-foreground rounded-bl-sm'
                      : 'bg-muted text-foreground border border-border rounded-bl-sm'
              }`}>
                {msg.sender === 'bot' && (
                  <div className="flex items-center gap-2 mb-2 opacity-70">
                    <Bot size={14} className={msg.isFake ? 'text-destructive' : msg.isFake === false ? 'text-success' : 'text-primary'} />
                    <span className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">AI System</span>
                  </div>
                )}
          ...
          {isTyping && (
             <div className="flex justify-start">
               <div className="bg-muted border border-border rounded-2xl rounded-bl-sm p-3.5 flex items-center gap-2 w-16 h-10 shadow-lg">
        ...
        <div className="p-3 bg-card border-t border-border">
          <div className="flex items-center gap-2 bg-input/40 border border-border rounded-xl p-1 pr-1.5 focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20 transition-all shadow-inner">
            <input
              type="text"
              value={input}
            ...
            <button 
              onClick={handleSend}
              disabled={!input.trim()}
              className="p-2 rounded-lg bg-primary text-primary-foreground disabled:opacity-50 disabled:cursor-not-allowed hover:bg-primary/90 transition-colors shadow-md"
            >
              <Send size={16} />
            </button>
          </div>
